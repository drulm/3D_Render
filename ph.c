...
/*----------------------------------------------------------------
	...
	(3 days to create so please be kind on things I did to 
	code it up fast)
	For X11 / Linux
----------------------------------------------------------------*/
...
while (1)
	...
	for (i=0; i<=objcnt; i++) 
	  {rot_obj(i);
	  obj[i].x += obj[i].dx; obj[i].y += obj[i].dy; obj[i].z += obj[i].dz;
	  obj[i].rx += obj[i].drx;
	  obj[i].ry += obj[i].dry; 
	  obj[i].rz += obj[i].drz;}
...
for (i=0; i<=plycnt; i++)
	...
	if (i % 100 == 0) fprintf(stderr,"precomputed %d/%d polygons\n",i,plycnt);
	st=poly[i].st; end=poly[i].end;
		for (j=st; j<=end; j++)
			{ p1 = etn[j].p1; p2 = etn[j].p2;
			idx[p1].poly = i; idx[p2].poly = i; 
			}
	}
pc2=-1;
...
st=poly[p].st; end=poly[p].end;
vx=vy=vz=0;
pps=-1;
LIGHT=0.0;
for (i=0; i<=objcnt; i++)
	if (obj[i].st <= p && obj[i].end >= p)  {ob = i; break;}
if (obj[ob].tx > 0.0)
	{
...
for (i=0; i<=lgcnt; i++)
	{
/*	new = nm[v].new;
*/	lx=lg[i].x-x; ly=lg[i].y-y; lz=lg[i].z-z;
/*	lx=lg[i].x-ptn[new].x; ly=lg[i].y-ptn[new].y; lz=lg[i].z-ptn[new].z;
*/	vec.x=lx; vec.y=ly; vec.z=lz;
	m1=mag(&vec);
	lx/=m1; ly/=m1; lz/=m1;
	...
	LT = obj[ob].kd * lg[i].I * 
		(lx*vec.x + ly*vec.y + lz*vec.z);
	if (LT>0) LIGHT += LT;
/*	vx=0.0-ptn[new].x; vy=0.0-ptn[new].y; vz= 1000.0-ptn[new].z;
...
/*	LT = (obj[ob].ks * lg[i].I * 
		pow((hx*vec.x + hy*vec.y + hz*vec.z),obj[ob].kn));
*/	if (LT>0) LIGHT += LT;
	}
...
for (i=0; i<=lgcnt; i++)
	...
	m1=mag(&vec);
	lx/=m1; ly/=m1; lz/=m1;
	vec.x=nm[v].a; vec.y=nm[v].b; vec.z=nm[v].c;
	m1=mag(&vec);
	vec.x/=m1; vec.y/=m1; vec.z/=m1;
	LT = obj[ob].kd * lg[i].I * 
		(lx*vec.x + ly*vec.y + lz*vec.z);
...
for (i=st; i<=end; i++)
	{
	pt=etn[i].p1; in=0;
	for (j=0; j<=pps; j++) if (pt==pp[j]) in=1;
	if (!in) { pps++;  pp[pps]=pt;
		 vx += ptn[pt].x;  vy += ptn[pt].y;  vz += ptn[pt].z; }
	pt=etn[i].p2; in=0;
	for (j=0; j<=pps; j++) if (pt==pp[j]) in=1;
	if (!in) { pps++;  pp[pps]=pt;
		 vx += ptn[pt].x;  vy += ptn[pt].y;  vz += ptn[pt].z; }
	}
vx /= (pps+1); vy /= (pps+1); vz /= (pps+1);
for (i=0; i<=lgcnt; i++)
	{
	lx = lg[i].x-vx; ly = lg[i].y-vy; lz = lg[i].z-vz;
	vec.x=lx; vec.y=ly; vec.z=lz;
	m1=mag(&vec);
	lx/=m1; ly/=m1; lz/=m1;
	vec.x=poly[p].a; vec.y=poly[p].b; vec.z=poly[p].c;
	m1=mag(&vec);
	LT = obj[ob].kd * lg[i].I * 
		(lx*poly[p].a/m1 + ly*poly[p].b/m1 + lz*poly[p].c/m1);
	if (LT>0) LIGHT += LT;	
	}
if (LIGHT > 1.0) LIGHT=1.0;
...
while (!feof(fs) && com!='Q')
	...
	else {printf("ERROR in SCENE FILE!\n"); exit(-2);}
	}
}
...
for (k=0; k<=pntcnt; k++)
	{ nm[k].a=0.0; nm[k].b=0.0; nm[k].c=0.0; }
...
for (i=0; i<=plycnt; i++)
	...
	for (k=floor(miny)+1; k<=ceil(maxy)-1; k++)
		...
		for (j=0; j<c; j++)
			...
			xp1 = ((double)k-b)/m;
			if (xp1<minx && xp1>xx1 && 
			 (double)k>yy1 && (double)k<yy2) {minx=xp1;  ll=e[j];}
			if (xp1>maxx && xp1<xx2 && 
			 (double)k>yy1 && (double)k<yy2) {maxx=xp1;  rl=e[j];}
			}
		if (c==3 && (int)minx>0 && (int) maxx<TESTING_WIDTH
			&& ll != -1 && rl != -1)
		    { 
		    p1 = etn[ll].p1;  p2 = etn[ll].p2;
...
/*		      IL=((double)k-ptn[p2].y)/(ptn[p1].y-ptn[p2].y)
				*nm[idx[p1].org].l +
		         (ptn[p1].y-k)/(ptn[p1].y-ptn[p2].y)
				*nm[idx[p2].org].l;
*/		    else
			{
...
/*		      IR=((double)k-ptn[p2].y)/(ptn[p1].y-ptn[p2].y)
				*nm[idx[p1].org].l +
		         (ptn[p1].y-k)/(ptn[p1].y-ptn[p2].y)
				*nm[idx[p2].org].l;
*/		    else
...
*/ 
		IP = light3(i,RP.x,RP.y,RP.z,ppp.x,ppp.y,ppp.z);
			  ...
			  XSetForeground(display, topwinGc, 
			  	exact_defs[(int)IP].pixel);
...
*/	for (k=(int)miny; k<=(int)maxy; k++)
		...
		if (c==3 && (int)minx>0 && (int) maxx<TESTING_WIDTH)
			...
			if (dep>zbuff[j][k])
			  ...
			  XSetForeground(display, topwinGc, 
			     light(i));
			  XDrawPoint(display,toppix,topwinGc,
				j,k);
			  zbuff[j][k]=dep;
			  }
			}
...
for (i=0; i<=plycnt; i++)
	...
	p2 = etn[st].p1;
	p3 = etn[st+1].p1;
	if (p3==p1 || p3==p2) p3 = et[st+1].p2;
	x1 = ptn[p1].x; y1 = ptn[p1].y; z1 = ptn[p1].z;
	x2 = ptn[p2].x; y2 = ptn[p2].y; z2 = ptn[p2].z;
	x3 = ptn[p3].x; y3 = ptn[p3].y; z3 = ptn[p3].z;
	plane(x1,x2,x3,y1,y2,y3,z1,z2,z3,&aa,&bb,&cc,&dd);
	poly[i].a = aa; poly[i].b = bb;
	poly[i].c = cc; poly[i].d = dd;
	}
}
...
for (i=0; i<=edgecnt2; i++)
	...
	code1 = getcode(x,y,z); code2 = getcode(xp,yp,zp);
	if (code1 != 0 || code2 != 0)		/* clip! */
		if (code1 & code2)
			...
			if (nxt==0 || ck1 && ck2) etn[i].p1 = -1;
			else if (nxt==1 && !ck1)
				if (code1 && !code2)
				    {x=p[0].x; y=p[0].y; z=p[0].z;}
				else if (code2 && !code1)
				    {xp=p[0].x; yp=p[0].y; zp=p[0].z;}
				else etn[i].p1 = -1;
			else if (nxt==2 && !ck1 && !ck2)
		 		etn[i].p1 = -1;    
		 	else etn[i].p1 = -1;    
			p1=etn[i].p1; p2=etn[i].p2;
			if (etn[i].p1 != -1)
				{put_pnt(p1,x,y,z); put_pnt(p2,xp,yp,zp);}
			}
	}
}
...
int code;
code=0;
if (x < Cmin.x) code = code | 1;
if (x > Cmax.x) code = code | 2;
if (y < Cmin.y) code = code | 4;
if (y > Cmax.y) code = code | 8;
if (z < Cmin.z) code = code | 16;
if (z > Cmax.z) code = code | 32;
/* printf("%d\n",code); */
return(code);
}
	...
	if (poly[i].c>0.0)
		...
		for (j=st; j<=end; j++)
			if (etn[j].p1 != -1)
				...
				XDrawLine(display,toppix,topwinGc,
...
for (i=0; i<=seg; i++)
	...
	for (j=0; j<=pntcnt; j++)
		if (x==pt[j].x && y==pt[j].y && z==pt[j].z) 
			{in=0; oldin=j; j=pntcnt+1;}
	if (in) {pntcnt++;
		pt[pntcnt].x=x; pt[pntcnt].y=y; pt[pntcnt].z=z;
		e1=pntcnt;}
	   else e1=oldin; 
	in=1;
	for (j=0; j<=pntcnt; j++)
		if (xp==pt[j].x && yp==pt[j].y && zp==pt[j].z) 
			{in=0; oldin=j; j=pntcnt+1;}
	if (in) {pntcnt++;
		pt[pntcnt].x=xp; pt[pntcnt].y=yp; pt[pntcnt].z=zp;
		e2=pntcnt;}
	   else e2=oldin;
	edgecnt++; 
	...
	if (pc==1)
		...
		{ 
		poly[plycnt].end = edgecnt; pc=0; 
		p1 = et[poly[plycnt].st].p2;
		p2 = et[poly[plycnt].st].p1;
		p3 = et[poly[plycnt].st+2].p1;
		if (p3==p1 || p3==p2) p3 = et[poly[plycnt].st+2].p2;
		x1 = pt[p1].x; y1 = pt[p1].y; z1 = pt[p1].z; 
		x2 = pt[p2].x; y2 = pt[p2].y; z2 = pt[p2].z;
		x3 = pt[p3].x; y3 = pt[p3].y; z3 = pt[p3].z;
		plane(x1,x2,x3,y1,y2,y3,z1,z2,z3,&aa,&bb,&cc,&dd);
		poly[plycnt].a = aa; poly[plycnt].b = bb;
		poly[plycnt].c = cc; poly[plycnt].d = dd;
		}
	}
...
for (i=0; i<=edgecnt; i++)
    ...
    edgecnt2++; etn[edgecnt2].p1 = pntcnt2+1; etn[edgecnt2].p2 = pntcnt2+2;
    pntcnt2++; ptn[pntcnt2].x = x; ptn[pntcnt2].y = y; ptn[pntcnt2].z = z; 
    pntcnt2++; ptn[pntcnt2].x = xp; ptn[pntcnt2].y = yp; ptn[pntcnt2].z = zp; 
    }
}
...
double m;
m = mag(N);
n->x = N->x / m; n->y = N->y / m; n->z = N->z / m;
cross(V,N,u);
m = mag(u);
u->x = u->x / m; u->y = u->y / m; u->z = u->z / m;
cross(n,u,v);
}
...
{
double xp,yp,zp;
xp = *x * cos(az) - *y * sin(az);	/* Z AXIS! */
yp = *x * sin(az) + *y * cos(az);
*x = xp;	*y = yp; 
yp = *y * cos(ax) - *z * sin(ax);	/* X AXIS! */
zp = *y * sin(ax) + *z * cos(ax);
*y = yp;	*z = zp; 
zp = *z * cos(ay) - *x * sin(ay);	/* X AXIS! */
...
double *x1,*y1,*z1,*x2,*y2,*z2;
{ int v1,v2;
v1 = et[i].p1;  v2 = et[i].p2;
*x1= pt[v1].x ; *x2 = pt[v2].x; 
*y1= pt[v1].y ; *y2 = pt[v2].y; 
*z1= pt[v1].z ; *z2 = pt[v2].z; 
}
...
struct line *p;
double *x1,*y1,*z1,*x2,*y2,*z2;
{*x1 = p->x1 ; *x2 = p->x2; *y1 = p->y1; *y2 = p->y2; *z1 = p->z1; *z2 = p->z2;}

/*---------------------------------------------------------------------------*/
void set_line(p,x1,y1,z1,x2,y2,z2)
struct line *p;
double x1,y1,z1,x2,y2,z2;
{ p->x1 = x1; p->x2 = x2; p->y1 = y1; p->y2 = y2; p->z1 = z1; p->z2 = z2;}
...
toppix = XCreatePixmap(display, DefaultRootWindow(display),
		TESTING_WIDTH, TESTING_HEIGHT, 
